 ...
 * Projeto: vfhxggyhzxbyckbskbtg
 * URL: https://vfhxggyhzxbyckbskbtg.supabase.co
 */

var SUPABASE_CONFIG = {
  // URLs e Chaves
  url: 'https://vfhxggyhzxbyckbskbtg.supabase.co',
  anonKey: 'sb_publishable_5em8KqbT5Z6CxJnYPQHCAw_kopzOrh0',
  publishableKey: 'sb_publishable_5em8KqbT5Z6CxJnYPQHCAw_kopzOrh0',
  ...
  async getCategorias() {
    ...
    try {
      const { data, error } = await this.client
        .from(SUPABASE_CONFIG.tables.categorias)
        .select('id, nome')
        .order('nome', { ascending: true });
      // Não lançar erro se a tabela não existir, apenas retornar vazio
      if (error) {
        console.warn('⚠️ Categorias:', error.message);
        return [];
      }
      return data || [];
    } catch (err) {
    ...
    try {
      ...
      const { data: order, error: orderError } = await this.client
        ...
        .insert([{
          ...
          phone: cleanPhone,
          order_type: (pedido.tipo_pedido === 'retirada' ? 'delivery' : (pedido.tipo_pedido || 'delivery'))
        }])
        .select('id')
        .single();
